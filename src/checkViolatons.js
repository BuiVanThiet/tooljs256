...
async function processAccountGroup(group) {
    const promises = group.map(async (product, index) => {
        const toaDoX = index * 1200;

        const profileId = await getIdAcc(product["Name Acc"]);
        console.log(`Processing account: ${product["Name Acc"]}`);
        console.log(`Position for account ${product["Name Acc"]}: { x: ${toaDoX}, y: 0 }`);
        ...
        for (let i = 0; i < 100000; i++) {
            const numberOfRows = await page.$$eval('tbody tr.core-table-tr', rows => rows.length);
            const noSearch = await page.evaluate(() => {
                // Lấy nội dung từ phần tử trong trang web
                return document.querySelector('div.pulse-empty div.pulse-empty-description')?.innerText.trim() || 'Unknown';
            });

            if (noSearch === 'No search results') {
                console.log('No search results found');
                break; // Dừng vòng lặp hoặc thực hiện hành động khác
            }
            ...
            for (let i = 0; i < numberOfRows; i++) {
                ...
                const checkNameProduct = await waitForElement(page, 'div.flex.flex-col div.text-p3-regular.text-neutral-text1.cursor-default div.flex.mt-4 div.ml-8.text-p3-regular p:nth-child(1)', 5000);
                if (checkNameProduct) {
                    const modalData = await page.evaluate(() => {
                        const productName = document.querySelector('div.flex.flex-col div.text-p3-regular.text-neutral-text1.cursor-default div.flex.mt-4 div.ml-8.text-p3-regular p:nth-child(1)')?.innerText.trim() || 'Unknown';
                        const productId = document.querySelector('div.flex.flex-col div.text-p3-regular.text-neutral-text1.cursor-default div.flex.mt-4 div.ml-8.text-p3-regular p:nth-child(2)')?.innerText.trim() || 'Unknown';
        ...
        if (products.length > 0) {
            const combinedData = products.map((product, index) => {
                const modal = productModal[index];

                if (!modal) {
                    console.log(`Modal data for product at index ${index} is missing.`);
                    return {
                        ...product,  // Giữ lại tất cả các trường từ productData
                        productName: 'Unknown',  // Nếu không có modal, gán 'Unknown' cho tên sản phẩm
                        productId: 'Unknown'    // Nếu không có modal, gán 'Unknown' cho ID sản phẩm
                    };
                }
                ...
                return {
                    ...product,
                    productName: modal.productName,  // Thêm tên sản phẩm từ modal
                    productId: modal.productId.replace('Product ID:', '')      // Thêm ID sản phẩm từ modal
                };
            });
